/* Сравнение 2-х автоматов Мили.
   ...
   6 - 10) аналогично 1 - 5, но для второго автомата.
   
   Выходные данные:
   1) EQUAL - если автоматы эвиваленты, иначе NOT EQUAL.
*/
...
func main() {
    ...
    flag_ := [50]bool{}
    var start_ int
    reNumerate(δ_, q0_, m_, &visit_, &flag_, &start_)
    ...
    for i := 0; i < n_; i++ {
        ...
        for j := 0; j < m_; j++ {
            δ1_[visit_[i]][j] = visit_[δ_[i][j]]
            φ1_[visit_[i]][j] = φ_[i][j]
        }
    }
    
    if start == start_ && m == m_ {
        for i := 0; i < start; i++ {
            for j := 0; j < start; j++ {
                if δ1_[i][j] != δ1[i][j] || φ1_[i][j] != φ1[i][j] {
                    fmt.Print("NOT EQUAL");
                    return
                }
            }
